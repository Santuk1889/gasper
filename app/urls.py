...
urlpatterns = [
    ...
    path(
        "movimientos/crear/",
        views.CrearMovimientoView.as_view(),
        name="crear_movimiento",
    ),
    path("movimientos/", views.VerMovimientosView.as_view(), name="ver_movimientos"),
    path(
        "movimientos/detalle/<int:pk>/",
        views.DetalleMovimientoView.as_view(),
        name="detalle_movimiento",
    ),
    path(
        "movimientos/editar/<int:pk>/",
        views.EditarMovimientoView.as_view(),
        name="editar_movimiento",
    ),
    path(
        "movimientos/eliminar/<int:pk>/",
        views.EliminarMovimientoView.as_view(),
        name="eliminar_movimiento",
    ),  # Nueva URL para eliminar movimientos
    path(
        "categorias/crear/", views.CrearCategoriaView.as_view(), name="crear_categoria"
    ),
